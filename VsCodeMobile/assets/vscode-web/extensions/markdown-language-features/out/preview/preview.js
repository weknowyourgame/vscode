...
class MarkdownPreview extends dispose_1.Disposable {
    ...
    _opener;
    static _unwatchedImageSchemes = new Set(['https', 'http', 'data']);
    _disposed = false;
    _delay = 300;
    _throttleTimer;
    _resource;
    ...
    _isScrolling = false;
    _imageInfo = [];
    _fileWatchersBySrc = new Map();
    _onScrollEmitter = this._register(new vscode.EventEmitter());
    onScroll = this._onScrollEmitter.event;
    _disposeCts = this._register(new vscode.CancellationTokenSource());
...
class StaticMarkdownPreview extends dispose_1.Disposable {
    ...
    _onDispose = this._register(new vscode.EventEmitter());
    onDispose = this._onDispose.event;
    _onDidChangeViewState = this._register(new vscode.EventEmitter());
...
class DynamicMarkdownPreview extends dispose_1.Disposable {
    ...
    _onDidChangeViewStateEmitter = this._register(new vscode.EventEmitter());
