...
async function startClientWithParticipants(_context, languageParticipants, newLanguageClient, runtime) {
    ...
    client.onRequest(VSCodeContentRequest.type, async (uriPath) => {
        ...
        if (uri.scheme === 'vscode') {
        ...
        else if (schemaDownloadEnabled && vscode_1.workspace.isTrusted) {
            ...
            if (!vscode_1.workspace.isTrusted) {
                throw new vscode_languageclient_1.ResponseError(1, vscode_1.l10n.t('Downloading schemas is disabled in untrusted workspaces'));
            }
    ...
    toDispose.push(vscode_1.workspace.onDidGrantWorkspaceTrust(updateSchemaDownloadSetting));
    ...
    function updateSchemaDownloadSetting() {
        if (!vscode_1.workspace.isTrusted) {
            schemaResolutionErrorStatusBarItem.tooltip = vscode_1.l10n.t('Unable to download schemas in untrusted workspaces.');
